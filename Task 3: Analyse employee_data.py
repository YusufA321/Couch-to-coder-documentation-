...
# 1. Print the name of the person who has the highest salary at the company. Answer: Shawn
highest = employees[0]
...
# 2. Print the combined years of experience of all employees at the company. Answer: 183
total_years_of_exp = 0

for employee in employees:
    total_years_of_exp += employee["years_of_experience"]

print(total_years_of_exp)
...
# 4. Which one costs more for the company - Product department salaries or Business department salaries? Answer: Product Department
total_product_depart_salaries = 0

for employee in employees:
    if (employee["department"] == "Product"):
        total_product_depart_salaries += employee["salary"]


total_business_depart_salaries = 0

for employee in employees:
    if (employee["department"] == "Business"):
        total_business_depart_salaries += employee["salary"]


if (total_product_depart_salaries > total_business_depart_salaries):
    print("Product department salaries cost more for the company")
elif (total_business_depart_salaries > total_product_depart_salaries):
    print("Business department salaries cost more for the company")



# 5. What is the average salary for people over 30 years of age?
total_salaries_over_30 = 0
no_of_employees_over_30 = 0

for employee in employees:
    if(employee["age"] >= 30):
        total_salaries_over_30 += employee["salary"]
        no_of_employees_over_30 += 1

average_salary_over_30 = total_salaries_over_30 / no_of_employees_over_30
print(average_salary_over_30)


# 6. Create a new dict and calculate how many people are in the company with the same job title (HARD)
job_title_counter = {}
for employee in employees:
    if(employee["job_title"] in job_title_counter):
        job_title_counter[employee["job_title"]] += 1
